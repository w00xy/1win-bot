...
from database.models import winUser
from database.engine import session_maker

async def save_user_id_db(user_id):
    async with session_maker as session:
        try:
            # Проверяем, существует ли уже такой пользователь
            result = await session.execute(select(winUser).filter(winUser.user_id == user_id))
            existing_user = result.scalar_one_or_none()

            if existing_user is None:
                # Если пользователь не существует, вставляем новую запись
                await session.execute(insert(winUser).values(user_id=user_id))
